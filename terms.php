...
$stmt = $pdo->prepare("SELECT setting_value FROM settings WHERE tenant_id = ? AND setting_key='legal_terms_text'");
...
<title>תקנון — <?= htmlspecialchars($tenant['name']) ?></title>
...
<body>
<div class="site-header"><div class="brand">תקנון האתר</div><a href="site.php?slug=<?= urlencode($slug) ?>" class="icon-btn">✕</a></div>
